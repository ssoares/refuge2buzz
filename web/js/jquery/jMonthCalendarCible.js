...
(function($) {
	...
	var def = {
                    ...
                    locale:{
                        ...
                        abbreviatedMonthNames:["Jan", "Fév", "Mar", "Avr", "Mai", "Jui", "Jul", "Aou", "Sep", "Oct", "Nov", "Dec"]
                    },
                    useLocale : false,  /* Ajout par Sacha */
                    smallCalendar :false,  /* Ajout par Sacha */
                    arrowToEventContinuty :false,  /* Ajout par Sacha */
                    ...
                    onEventDayOver: function() {},
                    onEventDayOut: function() {},
	...
	var _initHeaders = function() {
		...
		var navRow = $('<tr><td colspan="7" style="padding:0px;"><div class="FormHeader MonthNavigation"></div></td></tr>');
		...
		if(def.navLinks.enableToday) { navHead.append(todayLink); }
                ...
                if(def.useLocale)
                    navHead.append($('<h2 class="MonthName"></h2>').append(def.locale.monthNames[_workingDate.getMonth()] + " " + _workingDate.getFullYear()));
                else
                    navHead.append($('<h2 class="MonthName"></h2>').append(Date.CultureInfo.monthNames[_workingDate.getMonth()] + " " + _workingDate.getFullYear()));
		...
		for (var i = def.firstDayOfWeek; i < def.firstDayOfWeek+7; i++)
                {
                    ...
                    if(def.useLocale)
                        var wordday = def.locale.dayNames[weekday];
                    else
	...
	$.J.DrawCalendar = function(dateIn)
		...
		for (var i = 0; i < _totalBoxes; i++) {
			...
			var atts = {'class':"DateBox" + (weekday == 0 || weekday == 6 ? ' Weekend ' : ''),
						'date':currentDate.toString("M/d/yyyy"),
                                                'id':currentDate.toString("MMddyyyy")
			};
			...
			dateBox.bind('click', { Date: currentDate.clone() }, def.onDayCellClick);
                        ...
                        if(def.smallCalendar)       // Ajout par Sacha 21/07/2011
                        {
                            dateBox.bind('mouseover', { Date: currentDate.clone() }, def.onEventDayOver);
                            dateBox.bind('mouseout', { Date: currentDate.clone() }, def.onEventDayOut);
                            ...
                            $.each(cEvents, function()
                            {
                                ...
                                if (this.EventID == eventId)
                                {
                                    ...
                                    dateBox.bind('mouseover', { Event: event }, def.onEventDayOver);
                                    dateBox.bind('mouseout', { Event: event }, def.onEventDayOut);
		...
		var a = $(_containerId);
		var cal = $('<table class="MonthlyCalendar" id="CalendarGrid" cellpadding="0" tablespacing="0"></table>').append(headerRow, tBody);
				...
				for (var i = istart; i <= iend; i++) {
					...
					if (toManyEvents) {
						...
						block.css({
							...
							"left" : pos.left + 1,
							"width" : (b.getLabelWidth()+0),
							"position" : "absolute" });

                                                /*"width" : (b.getLabelWidth() - 7), */
						b.vOffset += 23;

                                                // Ajout par Sacha 21/07/2011
                                                if(def.arrowToEventContinuty)
                                                {
                                                    if (continueEvent)
                                                    {
						...
						if (e) {
							var w = e.css("width");
                                                        ...
                                                        var wid = parseInt(w);
                                                        wid = wid - 7;                                                        
							e.css({ "width" : (wid + b.getLabelWidth()+8) });
							b.vOffset += 23;
							b.events.push(ev);
						}
					}
	...
	var _buildEventBlock = function(ev, weekNumber)
		...
		var block = $('<div class="Event ' + ev.site + '" id="Event_' + ev.EventID + '_' + weekNumber + '" eventid="' + ev.EventID +'"></div>');
	...
	$.J.ClearEventsOnCalendar = function() {
		_clearBoxEvents();
		$(".Event", $(_containerId)).remove();
		$(".MoreEvents", $(_containerId)).remove();
	}
	...
	$.J.ReplaceEventCollection = function(eventCollection) {
       ...
       /* $.J.SetContainer = function() {
            _containerId = "#jMonthCalendarSmall";
        }
*/
	...
	$.J.Initialize = function(containerID,options, events) {
		var today = new Date();
		_containerId = containerID ;//"#jMonthCalendarSmall";
...
//Date.CultureInfo={name:"en-US",englishName:"English (United States)",nativeName:"English (United States)",dayNames:["Dimanche", "Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi", "Samedi"],abbreviatedDayNames:["Dim", "Lun", "Mar", "Mer", "Jeu", "Ven", "Sam"],shortestDayNames:["Di", "Lu", "Ma", "Me", "Je", "Ve", "Sa"],firstLetterDayNames:["D", "L", "M", "M", "J", "V", "S"],monthNames:["Janvier", "Février", "Mars", "Avril", "Mai", "Juin", "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre"],abbreviatedMonthNames:["Jan", "Fév", "Mar", "Avr", "Mai", "Jui", "Jul", "Aou", "Sep", "Oct", "Nov", "Dec"],amDesignator:"AM",pmDesignator:"PM",firstDayOfWeek:0,twoDigitYearMax:2029,dateElementOrder:"mdy",formatPatterns:{shortDate:"M/d/yyyy",longDate:"dddd, MMMM dd, yyyy",shortTime:"h:mm tt",longTime:"h:mm:ss tt",fullDateTime:"dddd, MMMM dd, yyyy h:mm:ss tt",sortableDateTime:"yyyy-MM-ddTHH:mm:ss",universalSortableDateTime:"yyyy-MM-dd HH:mm:ssZ",rfc1123:"ddd, dd MMM yyyy HH:mm:ss GMT",monthDay:"MMMM dd",yearMonth:"MMMM, yyyy"},regexPatterns:{jan:/^jan(uary)?/i,feb:/^feb(ruary)?/i,mar:/^mar(ch)?/i,apr:/^apr(il)?/i,may:/^may/i,jun:/^jun(e)?/i,jul:/^jul(y)?/i,aug:/^aug(ust)?/i,sep:/^sep(t(ember)?)?/i,oct:/^oct(ober)?/i,nov:/^nov(ember)?/i,dec:/^dec(ember)?/i,sun:/^su(n(day)?)?/i,mon:/^mo(n(day)?)?/i,tue:/^tu(e(s(day)?)?)?/i,wed:/^we(d(nesday)?)?/i,thu:/^th(u(r(s(day)?)?)?)?/i,fri:/^fr(i(day)?)?/i,sat:/^sa(t(urday)?)?/i,future:/^next/i,past:/^last|past|prev(ious)?/i,add:/^(\+|after|from)/i,subtract:/^(\-|before|ago)/i,yesterday:/^yesterday/i,today:/^t(oday)?/i,tomorrow:/^tomorrow/i,now:/^n(ow)?/i,millisecond:/^ms|milli(second)?s?/i,second:/^sec(ond)?s?/i,minute:/^min(ute)?s?/i,hour:/^h(ou)?rs?/i,week:/^w(ee)?k/i,month:/^m(o(nth)?s?)?/i,day:/^d(ays?)?/i,year:/^y((ea)?rs?)?/i,shortMeridian:/^(a|p)/i,longMeridian:/^(a\.?m?\.?|p\.?m?\.?)/i,timezone:/^((e(s|d)t|c(s|d)t|m(s|d)t|p(s|d)t)|((gmt)?\s*(\+|\-)\s*\d\d\d\d?)|gmt)/i,ordinalSuffix:/^\s*(st|nd|rd|th)/i,timeContext:/^\s*(\:|a|p)/i},abbreviatedTimeZoneStandard:{GMT:"-000",EST:"-0400",CST:"-0500",MST:"-0600",PST:"-0700"},abbreviatedTimeZoneDST:{GMT:"-000",EDT:"-0500",CDT:"-0600",MDT:"-0700",PDT:"-0800"}};
